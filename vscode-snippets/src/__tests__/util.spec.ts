...
describe('util', () => {
	describe('findLine', () => {
		...
		it('finds line with end test', () => {
			const result = findLine(document, regex.widgetFactoryMiddlewareAlone, { endTest: regex.widgetFactoryEnd });
			expect(result).toEqual(document.lineAt(8));
		});

		it('does not find line with end test', () => {
			const result = findLine(document, /NOT ON ANY LINE/g, { endTest: regex.widgetFactoryEnd });
